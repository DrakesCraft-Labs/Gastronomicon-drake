...
public class DynaTechSetup {
    ...
    private static boolean register(SlimefunItem chamber, int seconds, ItemStack[] inputs, ItemStack[] outputs) {
        ...
        try {
            final Method register = chamber.getClass().getMethod(
                "registerRecipe", int.class, ItemStack[].class, ItemStack[].class);
            register.invoke(chamber, seconds, inputs, outputs);
            return true;
    ...
    /** Registers against whichever DynaTech chambers are truly present in this server build. */
    private static boolean register(int seconds, ItemStack... outputs) {
        if (outputs.length == 0) {
            return false;
        }
        ItemStack[] inputs = {outputs[0].asOne()};
        boolean registered = register(gc, seconds, inputs, outputs);
        ItemStack[] mk2Outputs = Arrays.stream(outputs)
